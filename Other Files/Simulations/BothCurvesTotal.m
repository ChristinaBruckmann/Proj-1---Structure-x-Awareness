...
%             percentaccuracyob=(0.5./(1+exp(-obslope*(currpoint-obmidpoint+params.noiseStd*randn))))+0.5; %objective curve between 0.5 and 1
    ...
    % Mean
    subjmean=mean(totalresultssu,1,'omitnan');
    objmean=mean(totalresultsob,1,'omitnan');
    ...
    %% Fit Model and Plot
    modelsu=fit(overallsamplepoints',subjmean',logisticFunsu);
    fittingErrorsu=modelsu.b-sumidpoint;

    modelob=fit(overallsamplepoints',objmean',logisticFunob);
