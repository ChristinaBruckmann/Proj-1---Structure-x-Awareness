...
elseif experimentrun && session_n==1
    screeninfo.scopetest=0; % disables also skip sync, speedrun, practice and threshold
    screeninfo.speedrun=1; % disables practice, threshold and quickpractice
    SkipSync=1;
...
elseif experimentrun && session_n==2
    ...
    initialpractice3=0; % Practice with circles and varying visibility
...
% Other
visStiminfo.gcontrasts = [0 linspace(0.1,1,mixinfo.nContrasts)]; % lin spaced contrasts for main experiment
...
%             io64(timeinfo.triggerPort, timeinfo.triggerPortAddress, 254);
%             pause(5)
%             io64(timeinfo.triggerPort, timeinfo.triggerPortAddress, 0);
            ...
            if b<=3
                breaktext=sprintf('Block %i/%i completed. Please take a break.\n \n\n \n Wait for the experimenter to start the next block.', b,(mixinfo.nBlockstot+2)); % Pretend it's more blocks
            else
                breaktext=sprintf('Block %i/%i completed. Please take a break.\n \n\n \n Press any button to start the next block.', b,(mixinfo.nBlockstot+2));
            end
...
%         pause(5)
%         io64(timeinfo.triggerPort, timeinfo.triggerPortAddress, 255);
%         pause(1)
%         io64(timeinfo.triggerPort, timeinfo.triggerPortAddress, 0);
        end
        % Save
...
for i=1:timeinfo.TarDurFr 
        randMask=(rand(visStiminfo.rectSize)-0.5)*2;
        afctargetStim=0.5+0.5*(visStiminfo.maskintensity*randMask+visStiminfo.gabor2afc*0.6); % input: between -1 and 1, output between 0 and 1)
...
for TarTime=1:timeinfo.TarDurFr
    DrawFormattedText(screeninfo.win,intervallabel,'center', screeninfo.yCenter-(0.25*screeninfo.axisy(2)),screeninfo.fontcolour);
    Screen('DrawTexture', screeninfo.win, visStiminfo.afcTarget(i));
    Screen('Flip', screeninfo.win);
    cwarn=cwarn+1;
end
