%% Structure x Awareness Two Session Version
% This script calculates psychometric curves for each session individually
% as well as both sessions merged.
% Both for individual and group level.
% Aim: See if we can reduce the amount of sessions, and if the results from
% the first session are stable enough
...
grouplevel=1; % Only plot group level results?

% Load
cd 'C:\Users\cbruckmann\Documents\PhD Projects\Proj1 - StructurexAwareness\SxA_TwoSessions\Data\Behavioural Preprocessed\Individual Sessions'
if grouplevel
    for ns=sessions
        for s=1:length(subj)
            if ns==1
                loadname=sprintf('SxA_ResultsSubject%i_Session1.mat',subj(s));
            elseif ns==2
                loadname=sprintf('SxA_ResultsSubject%i_Session2.mat',subj(s));
            elseif ns==3
        ...
        clear dataforpsignifit_total psignifitsresults midpoints
    end
else
    % Individual Subjects
end
