%% UPDATED Single Subject Analysis (Structure x Awareness)
% Includes Block Information

function New_SxA_SingleSubjBehav_Analysis(subj_n,session_n,plots)
...
if session_n==3
   session_number(1:440,1)=1;
   session_number(441:height(subresults.data))=2;
else
...
% Now does this separately for each block and includes a block identifier in the output
for condition=1:3
    contrast_count=1;
%     block_count=1;
    for sessions=1:2
    idxcondition=find((alldataclean{:,"Condition"} == condition) & (alldataclean{:,"Session"} == sessions)); % get date for condition and session
    currentdata=alldataclean(idxcondition,:);
%     unique_blocks=unique(currentdata{:,"Block"});
%     for block=1:length(unique_blocks)
        for ncont=1:10
            %idxcont=find(currentdata{:,"Block"} == (unique_blocks(block)) & currentdata{:,"Contrast Level"} == ncont);
            idxcont=find(currentdata{:,"Contrast Level"} == ncont);
            % Objective
            tempdataforpsignifit.obj(contrast_count,4)=sessions; % which block (block identifier numbers do not matter, so for ease of distinction I just add the session number here.
            tempdataforpsignifit.obj(contrast_count,3)=length(idxcont); % how many trials in total
            tempdataforpsignifit.obj(contrast_count,2)=length(find(currentdata{idxcont,"Correct/Incorrect"} == 1)); % how many correct
            tempdataforpsignifit.obj(contrast_count,1)=ncont; % contrast level

            % Subjective
            tempdataforpsignifit.subj(contrast_count,4)=sessions; 
            tempdataforpsignifit.subj(contrast_count,3)=length(idxcont);
            tempdataforpsignifit.subj(contrast_count,2)=length(find(currentdata{idxcont,"Binary Visibility"} == 1));
            tempdataforpsignifit.subj(contrast_count,1)=ncont;
