...
show_data = [VEOG HEOG SDATA.data(:,SDATA.metadata.good_electrodes)];
multichanplot(show_data, 10, 'srate', SDATA.info.sampling_rate, 'channelnames', clean_channels_labels_with_bipolar, 'ylim', [-100 100]);
...
for cutidx=1:length(breakstopidx)
    cutidxrev=flip(1:length(breakstopidx),2); % do it from the back so the indices do not change
    ToBeDeleted(breakstartidx(cutidxrev(cutidx))+5:breakstopidx(cutidxrev(cutidx))-100,:)=1; % remove data points but don't accidentally cut too much (e.g. baseline)
end

% Create anonymous triggers
AnonymousTriggers1=zeros(length(SDATA.events.triggerChannel),1);
for trigidx=1:length(SDATA.events.triggerChannel)
   if ismember(SDATA.events.triggerChannel(trigidx),[254 255]) % Start/End of block
        AnonymousTriggers1(trigidx)=2000;
...
if DeleteMarking
    delidx=find(ToBeDeleted==1);
    delidx=flip(delidx,2);
    SDATA.data(delidx,:)=[]; % remove data points
    SDATA.events.triggerChannel(delidx)=[]; % remove trigger points
end
