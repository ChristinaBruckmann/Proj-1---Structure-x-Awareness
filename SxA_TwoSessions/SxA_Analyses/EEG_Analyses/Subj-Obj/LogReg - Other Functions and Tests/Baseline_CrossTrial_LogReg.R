# Logistic Regression Alpha-Contrast-Perception - Baseline Comparison Version
# Similar to the LogisticRegressionR.R script, but only for interval and objective so we can redude the amount of data during
# testing baselines
...
for (s in subj) {
  readname=sprintf('~/PhD Projects/Proj1 - StructurexAwareness/SxA_TwoSessions/SxA_Data/EEG Results/SubjObj/BaselineTesting/RInput_LogReg_S%i_uc_dv.mat',s)
  ...
  #savefilename=sprintf('uc_dv_z_Subj%i_ObjectiveIntervalAlpha_contrast.txt',s)
  #write.table(contrast_coeff_objint, file = savefilename, row.names=FALSE)
  
  #savefilename=sprintf('uc_dv_z_Subj%i_ObjectiveIntervalAlpha_power.txt',s)
  ...
  savefilename=sprintf('uc_dv_Subj%i_SubjectiveIntervalAlpha_contrast.txt',s)
  write.table(contrast_coeff_subjint, file = savefilename, row.names=FALSE)
  
  savefilename=sprintf('uc_dv_Subj%i_SubjectiveIntervalAlpha_power.txt',s)
