...
%% Pre-Clean Data for Participant 14 (cut breaks and remove empty electrodes)
if partnum==14
...
%%
%% Function for participant 14 (remove the recordings during breaks) and remove additional empty channels
function [SDATA]=cleanpart4(SDATA)

% Delete unused electrodes
SDATA.data=SDATA.data(:,1:end-7);
SDATA.info.channel_labels=SDATA.info.channel_labels(1:end-7);


% Get block end indices from triggers
breakstartidx=find(SDATA.events.triggerChannel==255);

% Get indices from when the block starts again
for breakidx=1:length(breakstartidx)
    foundstart=0;
    currbreakstartidx=breakstartidx(breakidx);
    %Cycle through the following indices to search for the start of the next block
    cycleidx=currbreakstartidx;
    while ~foundstart
    cycleidx=cycleidx+1; 
    if SDATA.events.triggerChannel(cycleidx)~=0 && SDATA.events.triggerChannel(cycleidx)~=254 % skip the incorrect 254
    breakstopidx(breakidx)=cycleidx;
    foundstart=1;
    end
    end
end
SDATA.events.triggerChannel(find(SDATA.events.triggerChannel==254))=0; % Remove the incorrect starting block trigger

% Select to-be-deleted parts
ToBeDeleted=zeros(length(SDATA.events.triggerChannel),1);
for cutidx=1:length(breakstopidx)
    cutidxrev=str2num(fliplr(num2str(1:length(breakstopidx)))); % do it from the back so the indices do not change
    cutidxrev=cutidxrev(cutidx);
    ToBeDeleted(breakstartidx(cutidxrev)+5:breakstopidx(cutidxrev)-5,:)=1; % remove data points
end

% Inspect and Confirm Selection
show_data = [SDATA.data AnonymousTriggers1];
multichanplot(show_data, 10, 'srate', SDATA.info.sampling_rate, 'channelnames', [SDATA.info.channel_labels; {'Anonymous Trigger'}], 'ylim', [-100 100],'markdata',ToBeDeleted);
title('Original data')
DeleteMarking=input('Delete selected parts?(1=yes): ');

% Delete data collected in breaks and reassign 254 to mark the new block start
if DeleteMarking
    SDATA.data(DeleteMarking)=[]; % remove data points
    SDATA.events.triggerChannel(DeleteMarking)=[]; % remove trigger points
    for cutidx=1:length(breakstopidx)
        cutidxrev=str2num(fliplr(num2str(1:length(breakstopidx)))); % do it from the back so the indices do not change
        cutidxrev=cutidxrev(cutidx);
        SDATA.data(breakstartidx(cutidxrev)+5:breakstopidx(cutidxrev)-5,:)=[]; % remove data points
        SDATA.events.triggerChannel(breakstopidx(cutidxrev)-4)=254; % Add a starting block trigger where the block actually started
        SDATA.events.triggerChannel(breakstartidx(cutidxrev)+5:breakstopidx(cutidxrev)-5,:)=[]; % remove trigger points
    end
end
