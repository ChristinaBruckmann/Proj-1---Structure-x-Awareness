...
function [means]=calculate_means(alldataclean)
...
means.objmeanperPASrating(1)=mean(subresults.data{idx0,"Correct/Incorrect"});
means.objmeanperPASrating(2)=mean(subresults.data{idx1,"Correct/Incorrect"});
means.objmeanperPASrating(3)=mean(subresults.data{idx2,"Correct/Incorrect"});
means.objmeanperPASrating(4)=mean(subresults.data{idx3,"Correct/Incorrect"});
...
function [nrejected]=Remove_RT_Outliers(alldataclean)
...
%     figure(5); sp5(part)=subplot(3,4,part); histogram(outliercontrasts); title(figtitle5);
    ...
    % Save 
    save(resfilename, 'alldataclean_outliersrejec', 'red_dataforpsignifit' ,"-append")
end