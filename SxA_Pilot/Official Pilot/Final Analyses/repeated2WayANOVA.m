function [fValues, dfValues, pValues, effectMatrices]=repeated2WayANOVA(data,levelsA, levelsB)

% Two-way repeated measures ANOVA, for designs with two 'within' factors
% (both factors manipulated within the same levels of the random factor, e.g. subjects or electrodes).
%
% Inputs:
...
% e.g. if levelsA=3 and levelsB=2. data columns should be: A1B1 A1B2 A2B1 A2B2 A3B1 A3B2 
%
% Outputs:
% fValues - f values of the effects of A, B and the AB interaction
% dfValues - same for df
% pValues - same for p
% effectMatrices - estimations of the effect of each factor on each cell (necessary for permutation test)
