...
elseif experimentrun
    screeninfo.scopetest=1; % disables also skip sync, speedrun, practice and threshold
    screeninfo.speedrun=0; % disables practice, threshold and quickpractice
    SkipSync=0;
    timeinfo.recalcframes=1; % recalculate frames each trial with current frame rate?
    PAScheck=0; %ask for reason why they chose that PAS response during practice
...
% Other
visStiminfo.gcontrasts=logspace(log10(0.1),log10(1),mixinfo.nContrasts);
visStiminfo.thresholdcontrasts = flip(0.1:thresholdsteps:1);

% Trial info
targettrials=1:(length(visStiminfo.gcontrasts)); % minus one because we remove the catch trial contrast
...
%     else
        ...
        currcontrast=visStiminfo.gcontrasts(contrast); % choose gabor contrast for this trial (+1 because 0 cannot index, so everything is shifted)
%     end
end
